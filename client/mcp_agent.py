...
SYSTEM_PROMPT = (
    ...
    "- get_airport_schedules: Use when the user asks for DEPARTURES or ARRIVALS at an airport, "
    "e.g. 'show flights from Delhi', 'what flights leave Chennai today', 'arrivals at BOM'. "
    "Args: airport_iata (required), direction ('departures' or 'arrivals'), airline_iata (optional filter), limit.\n"
    "- get_airport_info: Use when the user asks about an AIRPORT — location, codes, timezone, coordinates. "
    "Args: search_term (airport name, city, or IATA code).\n"
    "- find_nearby_airports: Use when the user asks 'what airports are near [city/place]'. "
    "Args: lat, lng, radius_km.\n"
